---
title: "Biolector_RYC_Analysis_Markdown"
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 12)
...
raw_data <- read_excel(paste0(folder, file), sheet = "analysis", col_names = TRUE) %>%
  as.data.frame() %>%
  gather("time", "value", -c(well, channel)) %>%      #Make one single column with time info and a column with the value for that timepoint
...
raw_data <- raw_data %>%
  ...
  filter(!(sensor == "empty")) %>%    #if a replicate wants to be omitted, just put "empty" in the sensor column in the summary
  select(-well) %>%
  pivot_wider(names_from = "channel", values_from = "value")  #Give one column for each channel


#Subtract the background fluorescence from the Parental strain.

subtracted_data <- raw_data %>%
  group_by(medium, time) %>%    #Group everything except replicate and the column where you specify where the parental strain is!
  mutate(across(.cols = ends_with("_raw"), .fns = ~ .- mean(.[sensor == "Parental"]), .names = "{.col}_sub")) %>%
...
final_data <- subtracted_data %>%
  ...
  filter(time != min(subtracted_data$time)) %>%   #Remove first timepoint if it looks weird
  group_by(sensor, medium, replicate) %>%         #Group using all the variables except time
  mutate(YR.Norm = YR/first(YR)) %>%              #Normalise YR by first time value
  mutate(CR.Norm = CR/first(CR))                  #Normalise CR by first time value


#Computing mean and sd for all the desired columns.

columns_for_meansd <- setdiff(colnames(final_data), c(colnames(summary), "time"))  #Or make a vector with desired columns

summary_data <- final_data %>%
  group_by(sensor, medium, time) %>%    #Group everything except replicate!
  transmute(across(.cols = any_of(columns_for_meansd), .fns = list(mean = ~ mean(.x, na.rm = TRUE), sd = ~ sd(.x, na.rm = TRUE)))) %>%
  distinct()


#Save the data frame now if you are not computing specific growth rates, etc. 

sheets <- list("Data" = as.data.frame(final_data), "Summary_data" = as.data.frame(summary_data))
#write_xlsx(sheets, path = paste0(folder, "/Data_Analysed.xlsx"), use_zip64 = TRUE)

rm(subtracted_data, raw_data, sheets, columns_for_meansd, summary)     #Clean the R environment
```
...
#Replace the parameters you want to check in the vector in the loop (line 161).

tmp_plot <- final_data %>% pivot_longer(!c(sensor, medium, time, replicate), names_to = "Parameter", values_to = "value")    #data-frame rearrangement for easier plotting
  
for(i in c("biomass", "rfp", "cfp", "yfp")) {
  assign(paste0("plot_rep_", i),
         ggplot(data = subset(tmp_plot, Parameter == i)) + 
           ...
           theme_light()+
           theme(legend.position = "right", strip.background = element_rect(colour = "white", fill = "black")))
}

mget(ls()[startsWith(ls(), "plot_rep_")])       #Show all the plots checking the replicates
...
ggplot(many_spline_fits@fits) + geom_line()
...
growth_param <- bind_cols(growth_param, inf_points) %>%
  ...
  pivot_longer(c(lag, mumax), names_to = "Parameter", values_to = "value")  #Re-organisation


#Re-order for plotting later on
...
sheets <- list("Data" = as.data.frame(final_data), "Summary_data" = as.data.frame(summary_data), "Growth_Param" = as.data.frame(growth_param), 
               "Strain_Compare" = as.data.frame(Stats_strain_compare), "Medium_Compare" = as.data.frame(Stats_medium_compare)) 
write_xlsx(sheets, path = paste0(folder, "/Data_Analysed.xlsx"), use_zip64 = TRUE)
...
for(i in c("lag", "mumax")) {
  ...
  assign(paste0(i, "_bar"), 
         ggplot(data = df1) + aes(x = medium, y = value, group = sensor) +
           ...
           stat_summary(aes(x = medium, y = value/2, shape = sensor), fun = mean, geom = "point", 
                        size = 3, position = position_dodge(width = 0.9), color = "black") +
           stat_pvalue_manual(data = subset(Stats_strain_compare, Parameter == i), label = "p.adj.signif",
                              remove.bracket = T, vjust = -0.5) +
           stat_pvalue_manual(data = subset(Stats_medium_compare, Parameter == i), label = "p.adj.signif",
...
ggsave(filename = "Supplementary Figure 1.png", plot = Supp_fig1, device = "png", path = folder, 
...
plot_data <- final_data %>%
  ...
  pivot_longer(!c(sensor, medium, time, replicate), names_to = "Parameter", values_to = "value") %>%
...
for(i in sensors) {
  assign(paste0(i, "_DG"), ggplot(data = subset(plot_data, sensor == i & medium %in% DG_group)) + plot_aes)
  assign(paste0(i, "_Stress"), ggplot(data = subset(plot_data, sensor == i & medium %in% Stressor_group)) + plot_aes)
}
...
ggsave(filename = "Supplementary Figure 2.png", plot = Supp_fig2, device = "png", path = folder, 
...
early_lag <- subset(plot_data, sensor != "Parental" & time == unique(plot_data$time)[2]) %>% 
  mutate(Phase = "early_lag")

late_lag <- distinct(plot_data[, c("sensor", "medium", "time")]) %>%
  filter(sensor != "Parental") %>%   #As fluorescence will be plotted, no need of Parental strain
  merge(., subset(growth_param, Parameter == "lag")[, c("sensor", "medium", "value")]) %>%
  mutate(diff = abs(time - value)) %>%
  group_by(sensor, medium) %>%
  filter(diff == min(diff)) %>%   #Find the timepoints that are the closest to the lag phase lengths
  select(-c(value, diff)) %>%
  merge(., plot_data) %>%   #Add the fluorescence information to the selected timepoints
...
exp <- distinct(plot_data[, c("sensor", "medium", "time")]) %>%
  ...
  group_by(sensor, medium) %>%
  filter(diff == min(diff)) %>%   #Find the timepoints that are the closest to the inflection point
  select(-c(x, diff)) %>%
  merge(., plot_data) %>%   #Add the fluorescence information to the selected timepoints
  mutate(Phase = "exponential")

statio <- subset(plot_data, sensor != "Parental" & time == max(unique(plot_data$time))) %>% #last point in the screening
...
barplot_data$value[barplot_data$sensor == "PyruEth" & barplot_data$replicate == 3 & barplot_data$medium == "2DG_0.5mM" & barplot_data$Phase == "exponential" & barplot_data$Parameter == "CR.Norm"] <- 0.071 #Modify one value to allow for statistical analysis (from 0.07 to 0.071)
...
barplot_data <- merge(barplot_data, Stats_barplot_compare[, c("sensor", "Parameter", "IntraPar", "Phase", "p.adj.signif", "medium")], all.x = TRUE)  #Add significance to the data frame for plotting
...
barplot_aes <- list(aes(x = medium, y = value),
                    stat_summary(fun = mean, geom = "bar", aes(fill = medium), size = 1, position = position_dodge(width = 0.9)), 
                    stat_summary(fun.data = mean_sdl, geom = "errorbar", fun.args = list(mult = 1),
                                 width = 0.1, position = position_dodge(width = 0.9)),
                    stat_summary(aes(label = p.adj.signif, vjust = -0.5), fun = max, geom = "text"),
                    scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)), labels = function(x) format(x, nsmall = 1)),
                    scale_fill_manual(breaks = c("Delft", "2DG_0.5mM", "2DG_1mM", "2DG_2mM", "2DG_3mM", "Furfural_2g/L", "Acetate_6g/L","Ethanol_20g/L"), 
                                      values = c("#5A5A5A", "#fef0d9", "#fdcc8a", "#fc8d59", "#d7301f", "#A45BBD", "#c2a5cf", "#a6dba0")),
                    facet_grid(rows = vars(IntraPar), cols = vars(Phase), scales = "free_y"),
                    theme_light(),
...
for(i in sensors) {
  assign(paste0(i, "_bar"), 
         ggplot(data = subset(barplot_data, Parameter != "biomass" & sensor == i)) + barplot_aes)
  }
```
...
DG_fig2 <- ggplot(data = subset(plot_data, Parameter == "biomass" & sensor == "PyruEth" & medium %in% DG_group)) + plot_aes +
  stat_summary(data = subset(barplot_data, Parameter == "biomass" & sensor == "PyruEth" & medium %in% DG_group),
               fun = mean, geom = "point", aes (x = time, y = value, shape = Phase, group = medium), size = 3, color = "blue") 

Stress_fig2 <- ggplot(data = subset(plot_data, Parameter == "biomass" & sensor == "PyruEth" & medium %in% Stressor_group)) + plot_aes +
  stat_summary(data = subset(barplot_data, Parameter == "biomass" & sensor == "PyruEth" & medium %in% Stressor_group),
               fun = mean, geom = "point", aes (x = time, y = value, shape = Phase, group = medium), size = 3, color = "blue") 

fig2 <- ggarrange(ggarrange(DG_fig2, Stress_fig2, ncol = 2, nrow = 1, labels = c("A", "B")), 
                  RibUPR_bar, PyruEth_bar,
                  ncol = 1, nrow = 3, labels = c("", "C", "D"), heights = c(0.6, 1, 1))

ggsave(filename = "Figure 2.png", plot = fig2, device = "png", path = folder, width = 30, height = 40, unit = "cm", dpi = 300)
...
for(i in c("rmarkdown", requiredPackages)) {print(i); print(citation(i), style = "text"); cat('\n')}
